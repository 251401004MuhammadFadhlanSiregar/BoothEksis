...
void CameraPage::setupUI()
{
    ...
    m_orientBtn0->setStyleSheet(
        ...
        "  padding: 3px 10px;"
        "}"
        "QPushButton:hover { background: #EEE8FF; }"
        );
    connect(m_orientBtn0, &QPushButton::clicked, this, [this]() {
        m_landscape0 = !m_landscape0;
        recalcSlotRects();
        updateOrientButtons();
        if (m_currentSlot == 0)
    ...
    connect(m_orientBtn1, &QPushButton::clicked, this, [this]() {
        m_landscape1 = !m_landscape1;
        recalcSlotRects();
        updateOrientButtons();
        if (m_currentSlot == 1)
...
void CameraPage::onImageCaptured(int /*id*/, const QImage &img)
{
    ...
    cp.drawPixmap(0, 0, scaled,
                  (scaled.width()  - targetRect.width())  / 2,
                  (scaled.height() - targetRect.height()) / 2,
                  targetRect.width(), targetRect.height());
    cp.end();

    m_photos[m_currentSlot] = cropped;
    update();
...
void CameraPage::handleUploadMode()
{
    ...
    // Crop foto 0 sesuai orientasi slot 0
    {
        ...
        QPainter cp(&cropped);
        cp.drawPixmap(0, 0, scaled,
                      (scaled.width()  - m_slot0Rect.width())  / 2,
                      (scaled.height() - m_slot0Rect.height()) / 2,
                      m_slot0Rect.width(), m_slot0Rect.height());
        m_photos[0] = cropped;
    }
    ...
    // Crop foto 1 sesuai orientasi slot 1
    {
        ...
        QPainter cp(&cropped);
        cp.drawPixmap(0, 0, scaled,
                      (scaled.width()  - m_slot1Rect.width())  / 2,
                      (scaled.height() - m_slot1Rect.height()) / 2,
                      m_slot1Rect.width(), m_slot1Rect.height());
        m_photos[1] = cropped;
    }
