...
// GANTI seluruh fungsi setPhotos:
void StickerCanvas::setPhotos(const QVector<QPixmap> &photos)
{
    ...
    const int titleH      = 60;   // ruang judul atas
    const int gap         = 20;   // jarak antar slot
    const int padBottom   = 60;   // ruang dekorasi bawah

    // Ambil ukuran dari foto asli (portrait atau landscape)
    ...
    // Slot 0
    int x0 = (canvasW - s0.width()) / 2;
    m_slot0Rect = QRect(x0, titleH, s0.width(), s0.height());

    // Slot 1 dimulai tepat di bawah slot 0
    int x1 = (canvasW - s1.width()) / 2;
    m_slot1Rect = QRect(x1, m_slot0Rect.bottom() + gap,
                        s1.width(), s1.height());

    // Sesuaikan tinggi canvas
    int newH = m_slot1Rect.bottom() + padBottom;
    m_stripRect = QRect(0, 0, canvasW, newH);
    setFixedSize(canvasW, newH);
